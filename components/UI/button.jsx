    ...
    variants: {
      variant: {
        ...
        secondary:
          ...
          inscribirse: "bg-cs-lightOrange border border-input hover:bg-transparent hover:text-cs-lightOrange hover:border-cs-lightOrange text-cs-darkBlue",
...
const Button = React.forwardRef(({ className, variant, size, asChild = false,  rSize = [], rFontSize = [], ...props }, ref) => {
  ...
  return (
    <Comp
      className={cn(buttonVariants({ variant, size, className }), responsiveClasses)}
      ref={ref}
      {...props} />
  );
})
